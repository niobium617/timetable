/**
 * officialHolidays.js —— 官方节假日安排（按年份组织）
 *
 * 数据来源：国务院办公厅《关于部分节假日安排的通知》
 * - 2026：2025-11-04 通知（国办发明电〔2025〕7号）
 *   https://www.gov.cn/zhengce/zhengceku/202511/content_7047091.htm
 *   元旦 1/1-1/3；春节 2/15-2/23；清明 4/4-4/6；劳动节 5/1-5/5；
 *   端午 6/19-6/21；中秋 9/25-9/27；国庆 10/1-10/7，共 33 天。
 *   调休上班日：1/4、2/14、2/28、5/9、9/20、10/10（均为周末）。
 * - 2027：国务院办公厅预计 2026 年 10-11 月公布，公布后在下方补充数据即可，
 *   设置页按钮与示例数据按年份自动适配，无需改结构。
 *
 * 官方通知未定义调休上班日"补星期X的课"（各单位自行安排），targetWeekday
 * 由应用按附近假期推算（suggestAdjustWeekday），用户按学校通知核对改选。
 */

/** 各年份官方假期（key = 年份字符串） */
export const OFFICIAL_HOLIDAYS = {
	'2026': [
		{ date: '2026-01-01', name: '元旦' },
		{ date: '2026-01-02', name: '元旦' },
		{ date: '2026-01-03', name: '元旦' },

		{ date: '2026-02-15', name: '春节' },
		{ date: '2026-02-16', name: '春节' },
		{ date: '2026-02-17', name: '春节' },
		{ date: '2026-02-18', name: '春节' },
		{ date: '2026-02-19', name: '春节' },
		{ date: '2026-02-20', name: '春节' },
		{ date: '2026-02-21', name: '春节' },
		{ date: '2026-02-22', name: '春节' },
		{ date: '2026-02-23', name: '春节' },

		{ date: '2026-04-04', name: '清明节' },
		{ date: '2026-04-05', name: '清明节' },
		{ date: '2026-04-06', name: '清明节' },

		{ date: '2026-05-01', name: '劳动节' },
		{ date: '2026-05-02', name: '劳动节' },
		{ date: '2026-05-03', name: '劳动节' },
		{ date: '2026-05-04', name: '劳动节' },
		{ date: '2026-05-05', name: '劳动节' },

		{ date: '2026-06-19', name: '端午节' },
		{ date: '2026-06-20', name: '端午节' },
		{ date: '2026-06-21', name: '端午节' },

		{ date: '2026-09-25', name: '中秋节' },
		{ date: '2026-09-26', name: '中秋节' },
		{ date: '2026-09-27', name: '中秋节' },

		{ date: '2026-10-01', name: '国庆节' },
		{ date: '2026-10-02', name: '国庆节' },
		{ date: '2026-10-03', name: '国庆节' },
		{ date: '2026-10-04', name: '国庆节' },
		{ date: '2026-10-05', name: '国庆节' },
		{ date: '2026-10-06', name: '国庆节' },
		{ date: '2026-10-07', name: '国庆节' },
	],
	// '2027': 官方公布后补充
};

/** 各年份官方调休上班日（补课星期官方未定义，由应用推算，用户核对） */
export const OFFICIAL_ADJUSTMENTS = {
	'2026': [
		{ date: '2026-01-04', remark: '官方调休上班日（元旦），补课星期请按学校通知核对' },
		{ date: '2026-02-14', remark: '官方调休上班日（春节），补课星期请按学校通知核对' },
		{ date: '2026-02-28', remark: '官方调休上班日（春节），补课星期请按学校通知核对' },
		{ date: '2026-05-09', remark: '官方调休上班日（劳动节），补课星期请按学校通知核对' },
		{ date: '2026-09-20', remark: '官方调休上班日（国庆节），补课星期请按学校通知核对' },
		{ date: '2026-10-10', remark: '官方调休上班日（国庆节），补课星期请按学校通知核对' },
	],
	// '2027': 官方公布后补充
};

/** 已有数据的年份列表（升序），设置页按钮按此渲染 */
export function getOfficialYears() {
	return Object.keys(OFFICIAL_HOLIDAYS)
		.filter((y) => Array.isArray(OFFICIAL_HOLIDAYS[y]) && OFFICIAL_HOLIDAYS[y].length > 0)
		.sort();
}
